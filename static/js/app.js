...
function applyLanguage() {
    ...
    if ($('modalSummaryHeaderTitle')) $('modalSummaryHeaderTitle').textContent = currentLang === 'ru' ? 'ИИ-обзор репозитория и руководство' : 'AI Repository Summary & Dev Guide';
    if ($('btnGenDevPrompt')) $('btnGenDevPrompt').textContent = currentLang === 'ru' ? '📝 Промпт для Cursor/ChatGPT' : '📝 Generate Dev Prompt';
...
function generateLocalDigest(top10) {
    const dateStr = new Date().toLocaleDateString(currentLang === 'ru' ? 'ru-RU' : 'en-US', { day: 'numeric', month: 'long', year: 'numeric' });
    let md = '';
    if (currentLang === 'ru') {
        md += `## 🔥 GitHub Trending Weekly — ${dateStr}\n\n`;
        md += `Вот дайджест самых популярных и трендовых репозиториев этой недели на основе локально загруженных данных.\n\n`;
        
        top10.forEach((r, idx) => {
            const name = r.full_name || r.name;
            const stars = (r.stargazers_count || r.stars || 0).toLocaleString();
            const forks = (r.forks_count || r.forks || 0).toLocaleString();
            md += `### ${idx + 1}. [${name}](${r.html_url || 'https://github.com/'+name})\n`;
            md += `* **Язык:** ${r.language || 'Не указан'} • **Звёзды:** ⭐ ${stars} • **Форки:** 🔍 ${forks}\n`;
            md += `* **Описание:** ${r.description || 'Описание отсутствует.'}\n\n`;
        });
        
        md += `### 📈 Тренды недели\n`;
        md += `Среди самых активных проектов преобладают решения на языке **${top10[0]?.language || 'различных языках'}**, демонстрируя устойчивый интерес сообщества.\n\n`;
        md += `*Дайджест сгенерирован локально (ИИ временно недоступен).*`;
    } else {
        md += `## 🔥 GitHub Trending Weekly — ${dateStr}\n\n`;
        md += `Here is the digest of the most popular trending GitHub repositories based on currently loaded data.\n\n`;
        ...
        top10.forEach((r, idx) => {
            ...
            const forks = (r.forks_count || r.forks || 0).toLocaleString();
            md += `### ${idx + 1}. [${name}](${r.html_url || 'https://github.com/'+name})\n`;
            md += `* **Language:** ${r.language || 'N/A'} • **Stars:** ⭐ ${stars} • **Forks:** 🔍 ${forks}\n`;
            md += `* **Description:** ${r.description || 'No description available.'}\n\n`;
        });
        
        md += `### 📈 Trends of the Week\n`;
        md += `Most active projects today utilize **${top10[0]?.language || 'various languages'}**, reflecting current developer interest in these domains.\n\n`;
        md += `*Generated by local assistant (AI service offline).*`;
    }
    return md;
}
...
async function generateDigest() {
    ...
    try {
        ...
        if (d.error) {
            console.warn("AI digest generation failed:", d.error);
            const localMd = generateLocalDigest(top10);
            lastDigestMarkdown = localMd;
            container.innerHTML = `
                <div style="background: rgba(239, 68, 68, 0.08); border: 1px solid rgba(239, 68, 68, 0.2); border-radius: 6px; padding: 10px 12px; margin-bottom: 16px; font-size: 0.8rem; color: var(--red);">
                    ⚠️ ${currentLang === 'ru' ? 'Дайджест сгенерирован локально (не удалось связаться с ИИ или превышена квота).' : 'Digest generated locally as a fallback (unable to contact AI or quota exceeded).'}
                </div>
                ${renderMarkdown(localMd)}
            `;
            return;
        }
        ...
        console.warn("AI digest generation error:", e);
        const localMd = generateLocalDigest(top10);
        lastDigestMarkdown = localMd;
        container.innerHTML = `
            <div style="background: rgba(239, 68, 68, 0.08); border: 1px solid rgba(239, 68, 68, 0.2); border-radius: 6px; padding: 10px 12px; margin-bottom: 16px; font-size: 0.8rem; color: var(--red);">
                ⚠️ ${currentLang === 'ru' ? 'Дайджест сгенерирован локально (ошибка сети).' : 'Digest generated locally as a fallback (network error).'}
            </div>
            ${renderMarkdown(localMd)}
        `;
    }
...
function generateDevPrompt() {
    if (!currentDetailsRepoObj) return;
    
    // Set the selected repo for AI Chat context
    aiSelectedRepo = currentDetailsRepoObj;
    
    // Open AI Chat sidebar/pane
    const pane = $('aiChatPane');
    if (pane) pane.classList.remove('closed');
    ...
    const repoName = aiSelectedRepo.full_name || aiSelectedRepo.name;
    const query = currentLang === 'ru'
        ? `Сгенерируй детальный промт для реализации аналогичного проекта "${repoName}" с нуля. Включи архитектуру, стек технологий, пошаговые инструкции и ключевые моменты, чтобы я мог использовать этот промт в Cursor, v0 или ChatGPT.`
        : `Generate a detailed coding prompt to implement a clone/alternative version of "${repoName}" from scratch. Include architecture, recommended tech stack, step-by-step instructions, and key features to implement so I can copy-paste this prompt directly into Cursor, v0, or Claude.`;
        
    // Close repository modal so user can see the AI agent sidebar generating the prompt
    closeModal();
